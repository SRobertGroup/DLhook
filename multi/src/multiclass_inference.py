"""Multiclass counterpart to `models.UNetInference.UNetInference`, used only
by `multi/evaluate_multiclass.py` to run a trained 4-class checkpoint over
full-size crops.
...
Note on RGB vs BGR: `data_loader.py` reads training crops via PIL
(`.convert("RGB")`) while this module (like UNetInference) reads crops via
cv2 (BGR). This is *not* a bug worth fixing: every crop in
...
from .loss_functions import align_output_to_target
from .model import head_from_state_dict
...
class MulticlassInference:
    ...
    def __init__(self, checkpoint_path, num_classes: int = 4, batch_size: int | None = None,
        ...
        # remainder multi/src/loss_functions.align_output_to_target already
        # crops off in training (see that function's docstring). Reused here
        ...
        # multi/src/model.py:head_from_state_dict.
        ...
        try:
            ...
            # model have a "module." prefix on every key (see src/model.py's
            # _strip_module_prefix) -- fall back to wrapping instead of
            # stripping, mirroring UNetInference._load_model exactly.
    ...
    def _run_batch(self, tiles):
        ...
        PatchDataset (data_loader.py) z-scores each training patch using
        only that patch's own pixels (mean/std pooled over the exact HxWxC
        array fed to the network, margin included) -- i.e. training always
        normalises exactly the one array about to enter the network, nothing
        more. `_plan_tiles` (via `_pad_to_min`/`_get_tile_coords`, both
        unchanged from UNetInference) reflect-pads the base image up to at
        least `in_size` *before* tiling, and `in_size` is by construction

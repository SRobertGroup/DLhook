...
`multi/src/data_loader.py` (training) and `models/multiclass_inference.py`
...
CLAUDE.md's multi/ boundary note. multi/src/normalization.py re-exports this
module unchanged so existing imports keep working.
"""
...
def zscore_normalize(array: np.ndarray, eps: float = DEFAULT_EPS) -> np.ndarray:
    ...
    `PatchDataset.__getitem__` (multi/src/data_loader.py) does to its training
    patches (`raw_patch.mean()`, `raw_patch.std()` called on the full HxWxC
    array), so callers that want training-equivalent statistics must pass this
